	...
	var shortStraw = function(points){
		...
		for (var i = 0; i < cornerPoints.length; i++) {
			if (cornerPoints[i].X === undefined) {
				cornerPoints[i].X = cornerPoints[i].x;
			}
			if (cornerPoints[i].Y === undefined) {
				cornerPoints[i].Y = cornerPoints[i].y;
			}
		}
		//JDP END
		//4. return corners.
	...
	shortStraw.determineResampleSpacing = function(points) {
        ...
        shortStraw.getCorners = function (points) {
        	...
        	//JDP BEGIN iStraw improvement: 4.1 straws modification
        	straws[1] = (shortStraw.distance(points[0],points[1+w])) * (2*w)/(w+1);
        	straws[2] = (shortStraw.distance(points[0],points[2+w])) * (2*w)/(w+2);
        	straws[points.length-2] = (shortStraw.distance(points[points.length-1],points[points.length-2-w])) * (2*w)/(w+1);
        	straws[points.length-3] = (shortStraw.distance(points[points.length-1],points[points.length-3-w])) * (2*w)/(w+2);
        	//JDP END
        	
        	//JDP BEGIN iStraw improvement: 4.1 t modification
        	//var t = shortStraw.median(straws) * shortStraw.MEDIAN_THRESHOLD;
        	var t = shortStraw.mean(straws) * shortStraw.MEDIAN_THRESHOLD;
        	//JDP END
	...
	shortStraw.pathDistance = function(points, a, b) {
		...
		//JDP BEGIN iStraws improvement: 4.1 t change
        shortStraw.mean = function(values) {
        	var sum = 0.0;
