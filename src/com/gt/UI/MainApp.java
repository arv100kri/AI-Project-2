...
import com.gt.gesture.proxy.GestureLikelihoodContainer;
...
//				status.setText("Best Match Gesture  " + bestMatchGesture);
	...
	ActionListener train_alBtns = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == btnGenerateCodebook) {
				status.setText("Generating codebook...");
				CodeBookExecutor exec = new CodeBookExecutor();
				exec.execute();
			}
			if (e.getSource() == btnTrainHmmModel) {
				status.setText("Generating HMM Models...");
				GenerateHMMExecutor exec = new GenerateHMMExecutor();
				exec.execute();
			}
		}
	};

	class CodeBookExecutor extends SwingWorker<String, Object> {
	       @Override
	       public String doInBackground() {
	           oprMed.generateCodeBook();
	           return "Codebook generation complete!";
	       }
	...
	class GenerateHMMExecutor extends SwingWorker<String, Object> {
	       @Override
	       public String doInBackground() {
	    	   oprMed.trainHMM();
