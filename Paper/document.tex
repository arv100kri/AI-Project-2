...
\cite{pizlo_perception_2001}. Briefly, an inverse perception problem, as
...
finite input. A screenshot of the system in recognition mode is shown in the following figure:
...
Our approach, at a high level, is a \emph{visual template match}. The first step
...
of the segmentation points for a unistroke gesture sequence is shown in Figure 1 above.
...
tightly coupled (and dependent upon) the accuracy of the individual gesture
...
templates, and 5 templates. We calculated the three aforementioned accuracy metrics for
...
simplified problem of 1-1 gesture template matching due to the ambiguity
introduced by a gesture sequence. During the training mode, only sequences of
length 1 are allowed, and DTW recognition excels.
...
99.78\% for sequences of length 2. The difference in approach (reflecting
different problem statements), is a key consideration when comparing these
numbers. Yang et al train Hidden Markov models for both individual gestures as
well as gesture sequences. Thus, it is clear that training on gesture sequences
...
constraint related to our problem definition is that of training
only individual gestures. A key insight is that training on gesture sequences would
allow us to determine the features that indicate a segmentation point. For example,
a slight pause between gestures in a gesture sequence (observed during the study). Thus,
the transition between gestures in a gesture sequence is critical
...
``proximal stimulus Y''. Phrased in this manner, a relevant technique ideal
for recovering hidden state is Hidden Markov Models. Potential future work
is to train HMMs on individual gestures and chain HMMs to score permutations of gesture
sequences.
%----------------------------------------------------------------------------------------
\section{References}
