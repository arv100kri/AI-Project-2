...
%The key features of law enforcement's
...
During system use, the user chooses gestures from the pre-defined gesture list (which can be extended by the user), trains the selected gestures by drawing them individually (usually not more than 5 times). Then the user 
draws a gesture sequence containing a number of arbitrary gestures which the system will try to recognize. For the purposes of this study, and also for user convenience we restrict the maximum number of gestures in the sequence to be 3, but the system will work for any finite length of the input sequence as well.
\end{comment}
...
%where each input is sampled over time. 
...
The purpose of this metric is to determine %where the incorrect number of gestures was identified, or perhaps 
where the segmentation routine did not ``correctly segment" the input sequence.
%each gesture, but where the segmentation correctly segmented part of the input gesture sequence.
...
%A comparison for our results of sequences of length 2 is with the results presented by 
...
\begin{comment}
%Removing this purely for space reasons%
Table 2 indicates that our algorithm was able to recognize the correct number of
gestures in input sequences of length 1 71.5\% of the time.
...
Additionally, comparing the segmentation percentages for lengths 2 and 3
with the corresponding Overall Accuracy rates in Table 1 reveals that
our segmentation algorithm is able to identify the correct number of gestures even 
in cases where it may not correctly identify each individual gesture exactly.
\end{comment}
...
%segmentation routine and the DTW algorithm. 
...
%The second important understanding is that training on gesture sequences is very likely
%to yield impressive accuracy gains as demonstrated by Yang et al.
